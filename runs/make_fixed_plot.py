...
def main():
    # Load flow results
    target_betas = np.load(f"Data/fixed_s{SAMPLE_SIZE}.npy")[:, 0]
    order0 = np.load(f"Data/order0_s{SAMPLE_SIZE}.npy")
    order1 = np.load(f"Data/order1_s{SAMPLE_SIZE}.npy")
    order2 = np.load(f"Data/order2_s{SAMPLE_SIZE}.npy")
    ...
    y0 = np.poly1d(coef0)
    print(f"order0: {coef0}")
    coef1 = np.polyfit(target_betas, np.std(order1, axis=1), deg=2)
    y1 = np.poly1d(coef1)
    print(f"order1: {coef1}")
    coef2 = np.polyfit(target_betas, np.std(order2, axis=1), deg=2)
    y2 = np.poly1d(coef2)
    ...
    # Make Plots
    # # Data Points
    plt.errorbar(target_betas - 0.01, np.std(order0, axis=1), yerr=np.sqrt(SAMPLE_SIZE - 1) * np.std(order0_err, axis=1), c="r", fmt="x", elinewidth=1)
    ...
    plt.errorbar(target_betas + 0.01, np.std(order2, axis=1), yerr=np.sqrt(SAMPLE_SIZE - 1) * np.std(order2_err, axis=1), c="g", fmt="x", elinewidth=1)
